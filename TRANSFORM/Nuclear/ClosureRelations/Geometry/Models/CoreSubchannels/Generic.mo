...
model Generic
  ...
  input SI.Length dimension = 4*crossArea/perimeter "Characteristic dimension (e.g., hydraulic diameter)" annotation(Dialog(group="Input Variables"));
  input SI.Area crossArea = pi*dimension*dimension/4 "Cross-sectional flow areas" annotation(Dialog(group="Input Variables"));
  input SI.Length perimeter=4*crossArea/dimension "Wetted perimeters" annotation(Dialog(group="Input Variables"));
  input SI.Length length = 1.0 "Pipe length" annotation(Dialog(group="Input Variables"));
  input SI.Height roughness = 2.5e-5 "Average heights of surface asperities" annotation(Dialog(group="Input Variables"));
  input SI.Area surfaceArea[nSurfaces]={2*pi*rs_outer[end]*length*nPins}
    "Area per transfer surface" annotation (Dialog(group="Input Variables"));
  ...
  input SI.Length rs_outer[nRegions]={if i == 1 then 0.01 else rs_outer[i-1] + 0.001 for i in 1:nRegions} "Outer radius of each region {r_outer[1],...,r_outer[n]}"
    annotation (Dialog(group="Input Variables: Solid Media"));
