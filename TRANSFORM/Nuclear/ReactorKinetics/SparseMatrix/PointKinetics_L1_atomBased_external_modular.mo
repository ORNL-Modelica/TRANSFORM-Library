...
model PointKinetics_L1_atomBased_external_modular
  import TRANSFORM;
...
equation
  ...
  if specifyPower then
    ...
    if energyDynamics == Dynamics.SteadyState then
      0 = (rho - Beta)/Lambda*Q_fission + sum({w_f/(Lambda*nu_bar)*sum(lambdas .*
        mCs[i, :]) for i in 1:nV}) + w_f/(Lambda*nu_bar)*N_external;
    else
      der(Q_fission) = (rho - Beta)/Lambda*Q_fission + sum({w_f/(Lambda*nu_bar)*
        sum(lambdas .* mCs[i, :]) for i in 1:nV}) + w_f/(Lambda*nu_bar)*
        N_external;
    end if;
  end if;
  mC_gens = {{betas[j]*nu_bar/w_f*Q_fission*SF_Q_fission[i] - lambdas[j]*mCs[i,
    j] for j in 1:nC} for i in 1:nV};
  for i in 1:nV loop
    Qs[i] = Q_fission*SF_Q_fission[i];
  end for;
  annotation (
...
end PointKinetics_L1_atomBased_external_modular;
