...
model FissionProducts_externalBalance_withTritium_point
  ...
  input SIadd.ExtraPropertyExtrinsic[nC] mCs={0 for j in 1:nC}
  ...
  input SIadd.ExtraPropertyExtrinsic[nTR] mCs_TR={0 for j in 1:nTR}
    "Amount of each contributor to tritium [atoms]"
    annotation (Dialog(tab="Tritium Balance", group="Inputs"));

  Units.ExtraPropertyFlowRate mC_gens_H3[nTR]
  ...
  output TRANSFORM.Units.NonDim[nTR] rhos_TR "Tritium contributors reactivity feedback"
    annotation (Dialog(tab="Outputs", enable=false));
  output SIadd.ExtraPropertyFlowRate[nTR] mC_gens_TR "Generation rate of tritium contributors [atoms/s]"
    annotation (Dialog(
      tab="Outputs",
      enable=false));
...
equation
    ...
    for j in 1:nC loop
      mC_gens[j] = Qs_fission/w_f*sum({fissionSources[k]*sum({
        fissionTypes[k, m]*fissionYields[j, k, m] for m in 1:nT}) for k in 1:
        nFS}) - lambdas[j]*mCs[j] + sum(lambdas .* mCs[:] .* parents[j, :])
         - sigmasA[j]*mCs[j]*Qs_fission/(w_f*SigmaF)/Vs + (if j == iH3 then sum(mC_gens_H3[:]) else 0);
      rhos[j] = -sigmasA[j]*mCs[j]/(nu_bar*SigmaF)/Vs;
    end for;

  // Tritium sources from carrier fluid (e.g., FLiBe)
    for j in 1:nTR loop
       mC_gens_TR[j] =-lambdas_TR[j]*mCs_TR[j] + sum(lambdas_TR .* mCs_TR[
        :] .* parents_TR[j, :]) - (sigmasA_TR[j] + sigmasT_TR[j])*mCs_TR[j]*Qs_fission/(w_f*SigmaF)/Vs;
      rhos_TR[j] =-(sigmasA_TR[j] + sigmasT_TR[j])*mCs_TR[j]/(nu_bar*
        SigmaF)/Vs;
        end for;

  mC_gens_H3 ={sigmasT_TR[j]*mCs_TR[j]*Qs_fission/(w_f*SigmaF)/Vs
    for j in 1:nTR};
  ...
  Icon(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100},
          ...
          textString="FP")}));
end FissionProducts_externalBalance_withTritium_point;
