...
model Regions_3_FD2DCyl
  "2D Cylindrical Finite Difference model with three solid media regions"
  ...
  replaceable package Material_3 =
      TRANSFORM.Media.Interfaces.PartialAlloy "Region 3 material"
  annotation (choicesAllMatching=true);
  ...
  parameter Integer nR_3=3 "# nodes in region 2 radial direction";
  ...
  input SI.Length r_2_outer = r_1_outer + 0.001 "Outer radius of region 2"
    annotation (Dialog(group="Input Variables"));
  input SI.Length r_3_outer = r_2_outer + 0.001 "Outer radius of region 2"
    annotation (Dialog(group="Input Variables"));
  ...
  input SI.Length rs_2[nR_2]=linspace(
    ...
    annotation (Dialog(group="Input Variables"));
  input SI.Length rs_3[nR_3]=linspace(
      r_2_outer,
      r_3_outer,
      nR_3) "Region 3 radial positions"
    annotation (Dialog(group="Input Variables"));
  ...
  parameter SI.Temperature T_start_2=Material_2.T_default
    annotation (Dialog(tab="Initialization"));
  parameter SI.Temperature T_start_3=Material_3.T_default
    annotation (Dialog(tab="Initialization"));
  ...
  parameter SI.Temperature Ts_start_2[nR_2,nZ]=[{Ts_start_1[end, :]};
      ...
      nR_2 - 1,
      nZ)] annotation (Dialog(tab="Initialization"));
  parameter SI.Temperature Ts_start_3[nR_3,nZ]=[{Ts_start_2[end, :]};
      fill(
      T_start_3,
      nR_3 - 1,
      nZ)] annotation (Dialog(tab="Initialization"));
  ...
  TRANSFORM.HeatAndMassTransfer.DiscritizedModels.ClassicalMethod.BoundaryConditions.Adiabatic_FD
    adiabatic_FD3(nNodes=region_2.nR) annotation (Placement(transformation(
        ...
        origin={-1,-22})));
  TRANSFORM.HeatAndMassTransfer.DiscritizedModels.ClassicalMethod.BoundaryConditions.Adiabatic_FD
    adiabatic_FD4(nNodes=region_3.nR)
                                     annotation (Placement(transformation(
        extent={{-5,-4},{5,4}},
        rotation=-90,
        origin={33,22})));
  TRANSFORM.HeatAndMassTransfer.DiscritizedModels.ClassicalMethod.Cylinder_FD
    ...
    r_outer=r_3_outer,
    rs=rs_3,
    Tref(displayUnit="K") = T_start_3,
    Ts_start=Ts_start_3,
    redeclare package Material = Material_3)
    annotation (Placement(transformation(extent={{17,-17},{49,17}})));
  TRANSFORM.HeatAndMassTransfer.DiscritizedModels.ClassicalMethod.BoundaryConditions.Adiabatic_FD
    adiabatic_FD5(nNodes=region_3.nR) annotation (Placement(transformation(
        extent={{-5,-4},{5,4}},
        rotation=90,
        origin={33,-22})));
equation
  ...
  connect(region_2.heatPorts_outer, region_3.heatPorts_inner)
    annotation (Line(points={{8.44,0},{23.56,0}}, color={127,0,0}));
  connect(region_3.heatPorts_outer, scalePower.heatPorts_a)
    annotation (Line(points={{42.44,0},{51.22,0},{60,0}}, color={127,0,0}));
  connect(adiabatic_FD5.port, region_3.heatPorts_bottom) annotation (Line(
        points={{33,-17},{33,-14.5},{33,-10.03}}, color={191,0,0}));
  connect(adiabatic_FD4.port, region_3.heatPorts_top)
    annotation (Line(points={{33,17},{33,13.5},{33,10.37}}, color={191,0,0}));
