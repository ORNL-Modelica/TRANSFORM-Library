...
function fillNumString
  "Create array of strings with option to number"
  ...
  input Boolean number=true
    "=true to add extensions to string (i.e., {string_1, string_n...})";
...
algorithm
  if number then
    for i in 1:n loop
      arrayString[i] := Modelica.Utilities.Strings.replace(
          string + "_" + Modelica.Math.Vectors.toString({iNum}),
          " ",
          "");
      iNum := iNum + 1;
    end for;
