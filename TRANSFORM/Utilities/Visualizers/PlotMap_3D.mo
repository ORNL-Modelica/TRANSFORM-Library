...
model PlotMap_3D
  ...
  Real z_pixel "Coordinates in pixel space";
...
equation
  ...
  x_scaled = (x_pixel - anchor_origin[1])/(lowerRight                                             [1] - lowerLeft[1]);
  y_scaled = (y_pixel - anchor_origin[2])/(upperLeft[2] - lowerLeft[2]);
  y_scaled = (y_pixel - anchor_origin[2])/(upperLeft[2] - lowerLeft[2]);
  ...
  annotation (
    ...
    __Dymola_DymolaStoredErrors(thetext="model PlotMap_3D
...
equation 
  ...
  // Map the normalized values to pixel space
  -x_scaled*z_scaled  - = (x_pixel - anchor_origin[1])/(lowerRight[1] - lowerLeft[1]);
  y_scaled = (y_pixel - anchor_origin[2])/(upperLeft[2] - lowerLeft[2]);
  y_scaled = (y_pixel - anchor_origin[2])/(upperLeft[2] - lowerLeft[2]);
