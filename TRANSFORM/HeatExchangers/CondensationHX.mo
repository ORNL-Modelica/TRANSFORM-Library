...
model CondensationHX "Condensing HX"
...
equation
  ...
  connect(boundary1.port, volume_1_condensation.heatPort)
    annotation (Line(points={{-20,20},{-20,34}}, color={191,0,0}));
  connect(boundary1.Q_flow_ext, negate.y) annotation (Line(points={{-20,6},{
          -20,0},{-4.4,0}},color={0,0,127}));
  ...
  Q_flow_condensation =-volume_1_condensation.port_b[1].m_flow*(
  ...
  connect(sensor_h.port_a, port_a1)
    annotation (Line(points={{-80,40},{-100,40}}, color={0,127,255}));
  connect(volume_2_condensation.port_a[1], port_a2)
    annotation (Line(points={{26,-40},{100,-40}},  color={0,127,255}));
