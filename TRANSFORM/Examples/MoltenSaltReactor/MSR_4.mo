...
model MSR_4
  ...
  parameter TRANSFORM.Units.InverseTime lambda_i[nI]={0.0125,0.0318,0.109,0.317,
      1.35,8.64};
  SI.MassFlowRate[reflA_lower.nV,nI] mC_gens_reflA_lower = {{-lambda_i[j]*reflA_lower.mCs[i, j] for j in 1:nI} for i in 1:reflA_lower.nV};
  SI.MassFlowRate[fuelCell.nV,nI] mC_gens_fuelCell = kinetics.mC_gens;
  SI.MassFlowRate[reflA_upper.nV,nI] mC_gens_reflA_upper = {{-lambda_i[j]*reflA_upper.mCs[i, j] for j in 1:nI} for i in 1:reflA_upper.nV};
  SI.MassFlowRate[pipeToPHX_PFL.nV,nI] mC_gens_pipeToPHX_PFL = {{-lambda_i[j]*pipeToPHX_PFL.mCs[i, j] for j in 1:nI} for i in 1:pipeToPHX_PFL.nV};
  SI.MassFlowRate[PHX.tube.nV,nI] mC_gens_PHX_tube = {{-lambda_i[j]*PHX.tube.mCs[i, j] for j in 1:nI} for i in 1:PHX.tube.nV};
  SI.MassFlowRate[pipeFromPHX_PFL.nV,nI] mC_gens_pipeFromPHX_PFL = {{-lambda_i[j]*pipeFromPHX_PFL.mCs[i, j] for j in 1:nI} for i in 1:pipeFromPHX_PFL.nV};
