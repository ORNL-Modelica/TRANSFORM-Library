...
//   SI.Length drs[reflA_upperG.geometry.nR,reflA_upperG.geometry.nZ]={{rs[i+1,j] - rs[i,j] for j in 1:reflA_upperG.geometry.nZ} for i in 1:reflA_upperG.geometry.nR};
  ...
  parameter SIadd.ExtraProperty[kinetics.summary_data.data_TR.nC] C_start = N_A.*{1/Flibe_MM*MMFrac_LiF*Li6_molefrac,1/Flibe_MM*MMFrac_LiF*Li7_molefrac,1/Flibe_MM*(1-MMFrac_LiF),0}
    "atoms/kg fluid";
parameter SI.MassFraction Li7_enrichment = 0.99995
    "mass fraction Li-7 enrichment in flibe.  Baseline is 99.995%";
parameter SI.MoleFraction MMFrac_LiF = 0.67 "Mole fraction of LiF";
parameter SI.MolarMass Flibe_MM = 0.0328931
...
parameter SI.MoleFraction Li7_molefrac = (Li7_enrichment/Li7_MM)/((Li7_enrichment/Li7_MM)+((1.0-Li7_enrichment)/Li6_MM))
    "Mole fraction of lithium in flibe that is Li-7";
parameter SI.MoleFraction Li6_molefrac = 1.0-Li7_molefrac
...
//   SI.Temperature Tref_core[fuelCell.geometry.nV,kinetics.nFeedback] = {{if j == 1 then Tref_fuelCell[i] else Tref_fuelCellG[i] for j in 1:kinetics.nFeedback} for i in 1:fuelCell.geometry.nV};
  // Gathered info
  SI.Power Qt_total = sum(kinetics.Qs)
  ...
  SI.Temperature Ts_loop[1 + reflA_lower.nV + fuelCell.nV + reflA_upper.nV + 1
     + pipeToPHX_PFL.nV + PHX.tube.nV + pipeFromPHX_PFL.nV + 1] =                                                           cat(1,{plenum_lower.medium.T},reflA_lower.mediums.T,fuelCell.mediums.T,reflA_upper.mediums.T,
  ...
  SI.MassFlowRate m_flow_toDrainTank = data_OFFGAS.V_flow_sep_salt_total*Medium_PFL.density_ph(pump_PFL.port_b.p, pump_PFL.port_b.h_outflow)
  ...
  TRANSFORM.Utilities.Visualizers.Outputs.SpatialPlot2 spatialPlot2_1(
    ...
    x2=if PHX.counterCurrent == true then Modelica.Math.Vectors.reverse(PHX.shell.summary.xpos_norm) else PHX.shell.summary.xpos_norm,

    y2={PHX.shell.mediums[i].T for i in 1:PHX.geometry.nV},
    maxY1=max({data_PHX.T_inlet_tube,data_PHX.T_inlet_shell,data_PHX.T_outlet_tube,data_PHX.T_outlet_shell}),
    minY1=min({data_SHX.T_inlet_tube,data_SHX.T_inlet_shell,data_SHX.T_outlet_tube,data_SHX.T_outlet_shell}))
  ...
  TRANSFORM.Fluid.Machines.Pump_SimpleMassFlow pump_PCL(redeclare package Medium =
  ...
  TRANSFORM.Fluid.BoundaryConditions.Boundary_pT
    ...
    redeclare package Medium = Medium_BOP)
        ...
        rotation=0,
        origin={330,40})));
  TRANSFORM.Utilities.Visualizers.Outputs.SpatialPlot2 spatialPlot2_2(
    x1=SHX.tube.summary.xpos_norm,
    y1={SHX.tube.mediums[i].T for i in 1:SHX.geometry.nV},
    x2=if SHX.counterCurrent == true then Modelica.Math.Vectors.reverse(SHX.shell.summary.xpos_norm) else SHX.shell.summary.xpos_norm,

    y2={SHX.shell.mediums[i].T for i in 1:SHX.geometry.nV},
    minY1=min({data_SHX.T_inlet_tube,data_SHX.T_inlet_shell,data_SHX.T_outlet_tube,data_SHX.T_outlet_shell}),
    maxY1=max({data_SHX.T_inlet_tube,data_SHX.T_inlet_shell,data_SHX.T_outlet_tube,data_SHX.T_outlet_shell}))
  ...
  TRANSFORM.Fluid.FittingsAndResistances.SpecifiedResistance resistance_fromDrainTank(
    ...
    showName=systemTF.showName) annotation (Placement(transformation(
        ...
        rotation=0,
        origin={-220,-60})));
  TRANSFORM.Fluid.Machines.Pump_SimpleMassFlow pump_drainTank(redeclare package Medium =
  ...
  TRANSFORM.Fluid.TraceComponents.TraceSeparator traceSeparator(m_flow_sepFluid=
    ...
    iCar=iOG)                        annotation (Placement(transformation(
        ...
        rotation=90,
        origin={-270,90})));
  TRANSFORM.Fluid.Machines.Pump_SimpleMassFlow pump_bypass(redeclare package Medium =
               Medium_PFL, use_input=true)
...
equation
  ...
  connect(adsorberBed.port_a, drainTank_gas.port_b[1]) annotation (Line(points={{-230,-20},{-238,-20},{-238,-19.75},{-244,-19.75}},
                                                            color={0,127,255}));
  ...
  connect(pump_OffGas_adsorberBed.port_b, boundary_OffGas_sink.ports[2])
    annotation (Line(points={{-180,-20},{-158,-20},{-158,16},{-198,16},{-198,31},{-190,31}},
                      color={0,127,255}));
