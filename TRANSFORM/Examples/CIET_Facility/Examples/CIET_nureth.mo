...
model CIET_nureth "Final model of the CIET facility"
  ...
  Modelica.Units.SI.Power Q_insulation=sum(S7.boundary.port.Q_flow) +
      sum(S8.boundary.port.Q_flow) + sum(S9.boundary.port.Q_flow) + sum(
       S10.boundary.port.Q_flow) + sum(S12.boundary.port.Q_flow) + sum(
      S13.boundary.port.Q_flow) + sum(S14.boundary.port.Q_flow) + sum(
      S15.boundary.port.Q_flow) + sum(S1.boundary.port.Q_flow) + sum(S2.boundary.port.Q_flow)
       + sum(S3.boundary.port.Q_flow) + sum(S4.boundary.port.Q_flow) +
  ...
  Modelica.Units.SI.Power Q_C2H=sum(S12.boundary.port.Q_flow) + sum(S13.boundary.port.Q_flow)
       + sum(S14.boundary.port.Q_flow) + sum(S15.boundary.port.Q_flow) +
      sum(S1.boundary.port.Q_flow) + sum(S2.boundary.port.Q_flow) + sum(
       S3.boundary.port.Q_flow) + sum(S4.boundary.port.Q_flow) + sum(S5.boundary.port.Q_flow);
  ...
  Fluid.Machines.Pump_SimpleMassFlow pump_SimpleMassFlow(redeclare package Medium =
               Medium, m_flow_nominal=data.m_flow_primary)
