...
model v5
  ...
  HeatAndMassTransfer.BoundaryConditions.Heat.HeatFlow sink_AHX1(use_port=true)
    annotation (Placement(transformation(
        extent={{10,10},{-10,-10}},
        rotation=90,
        origin={-50,74})));
  ...
  Controls.LimPID PID(
    ...
    k=5e4) annotation (Placement(transformation(extent={{-66,86},{-56,96}})));
  Modelica.Blocks.Sources.RealExpression realExpression2(y=data.T_out_pcx)
...
equation
  ...
  connect(sensor_pT1.port, massFlowRate.port_b)
    annotation (Line(points={{40,-70},{40,-20}}, color={0,127,255}));
  ...
  connect(sensor_pT4.port, pump_SimpleMassFlow.port_a)
    annotation (Line(points={{-10,-64},{-10,-70},{0,-70}}, color={0,127,255}));
  connect(sensor_pT3.port, AHX.port_b)
    annotation (Line(points={{-80,-70},{-80,-60}}, color={0,127,255}));
  connect(sensor_pT5.port, RHX.port_b_shell)
    annotation (Line(points={{0,40},{0,45.4}}, color={0,127,255}));
  ...
  connect(realExpression2.y, PID.u_s) annotation (Line(points={{-73.4,94},{-70,
          94},{-70,91},{-67,91}}, color={0,0,127}));
  connect(sensor_pT2.T, PID.u_m) annotation (Line(points={{-91,0},{-98,0},{-98,
          82},{-61,82},{-61,85}}, color={0,0,127}));
  connect(PID.y, sink_AHX1.Q_flow_ext)
    annotation (Line(points={{-55.5,91},{-50,91},{-50,78}}, color={0,0,127}));
