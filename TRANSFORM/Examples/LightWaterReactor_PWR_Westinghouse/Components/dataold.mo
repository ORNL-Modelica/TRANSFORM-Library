...
record dataold
  ...
  parameter Modelica.Units.SI.MassFlowRate m_flow_nom_circulation=
      m_flow_feedWater_nominal*1.1
    "Nominal boiler circulation flow rate"
    annotation (Evaluate=true, Dialog(group="Flow"));
  // Temperature
  parameter Modelica.Units.SI.Temperature T_nom_feedWater=
      T_feedwater_nominal "Nominal feed water temperature"
  ...
  parameter Modelica.Units.SI.MassFlowRate m_flow_start_circulation=
    ...
    annotation (Evaluate=true, Dialog(group="Flow"));
  // Level
  parameter Modelica.Units.SI.Length boiler_level_start=0
    "Level start value"
    annotation (Evaluate=true, Dialog(group="Level"));
  // Riser vapor quality
  parameter Modelica.Units.SI.MassFraction riser_vaporQuality_start_out=
     0.83 "Riser outlet vapor quality"
    annotation (Evaluate=true, Dialog(group="Vapor quality"));
 // Temperature
  parameter Modelica.Units.SI.Temperature T_SG_water_inlet=
      T_SG_inlet_nominal "Steam generator inlet water temperature"
