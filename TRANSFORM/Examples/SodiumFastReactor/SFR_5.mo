...
model SFR_5
  ...
  UserInteraction.Outputs.SpatialPlot2 spatialPlot2_1(
    ...
    maxY1=max({data.T_IHX_inletIHTS,data.T_IHX_inletPHTS,data.T_IHX_inletIHTS,
        data.T_IHX_oultetPHTS}))
    annotation (Placement(transformation(extent={{148,-126},{198,-82}})));
  ...
  Fluid.BoundaryConditions.Boundary_pT boundary_dummy(
    ...
    nPorts=1) annotation (Placement(transformation(extent={{-32,22},{-40,30}})));
  ...
  HeatAndMassTransfer.Resistances.Heat.Convection convection[2](alpha={1000,10},
...
equation
  ...
  connect(shield.port_b, upperPlenum.port_a[1]) annotation (Line(points={{-100,
          -76},{-100,-20},{-59.25,-20}},     color={0,127,255}));
  ...
  connect(boundary7.ports[1], STHX.port_a_tube) annotation (Line(points={{100,
          -102},{88,-102},{88,-100},{24,-100},{24,-40}},color={0,127,255}));
  connect(boundary9.ports[1], STHX.port_b_tube) annotation (Line(points={{98,-2},
          {24,-2},{24,-20}},               color={0,127,255}));
  connect(resistance_toIHX1.port_b, STHX.port_a_shell) annotation (Line(points=
          {{-19,-6},{19.4,-6},{19.4,-20}}, color={0,127,255}));
  connect(lowerPlenum_outer.port_a[1], STHX.port_b_shell) annotation (Line(
        points={{16,-98},{19.4,-98},{19.4,-40}}, color={0,127,255}));
  connect(resistance_toExpTank.port_a, upperPlenum.port_b[1])
    annotation (Line(points={{-60,1},{-60,-8},{-59.5,-8}}, color={0,127,255}));
  connect(resistance_toIHX1.port_a, upperPlenum.port_b[2]) annotation (Line(
        points={{-33,-6},{-60.5,-6},{-60.5,-8}}, color={0,127,255}));
  ...
  connect(lowerPlenum.port_a[1:3], downcomer.port_b) annotation (Line(points={{
          -59.3333,-166},{-10,-166},{-10,-140}}, color={0,127,255}));
  ...
  connect(expansionTank.port_b, boundary_dummy.ports[1])
    annotation (Line(points={{-48,26},{-40,26}}, color={0,127,255}));
  connect(lowerPlenum_outer.port_b[1:3], pump.port_a) annotation (Line(points={
          {4,-97.3333},{0,-97.3333},{0,-32},{-10,-32},{-10,-40}}, color={0,127,
          255}));
  ...
  connect(lowerPlenum_outer.heatPort, convection[1].port_a)
    annotation (Line(points={{10,-104},{10,-140},{45,-140}}, color={191,0,0}));
  annotation (
    ...
    Icon(coordinateSystem(extent={{-100,-100},{100,100}}), graphics={
        ...
        Rectangle(
          ...
          origin={28,0},
          rotation=-90)}),
    experiment(StopTime=86400, __Dymola_NumberOfIntervals=8640));
end SFR_5;
