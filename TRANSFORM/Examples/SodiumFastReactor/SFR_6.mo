...
model SFR_6
 ...
 SI.HeatFlowRate Q_total_tube = sum(STHX.tube.heatTransfer.Q_flows);
  ...
  Fluid.Volumes.MixingVolume volume(
    nPorts_b=1,
    redeclare package Medium =
        Media.Fluids.Sodium.ConstantPropertyLiquidSodium,
    T_start=data.T_IHX_inletIHTS,
    nPorts_a=3)
    annotation (Placement(transformation(extent={{76,-110},{96,-90}})));

  Fluid.FittingsAndResistances.SpecifiedResistance resistances1[3](
                                                                  redeclare
      package Medium = Media.Fluids.Sodium.ConstantPropertyLiquidSodium, R={1/
        data.m_flow_IHX_IHTS,1/data.m_flow_IHX_IHTS,1/data.m_flow_IHX_IHTS})
    annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=180,
        origin={54,-100})));
  Fluid.FittingsAndResistances.SpecifiedResistance resistance_toExpTank1(
      redeclare package Medium =
        Media.Fluids.Sodium.ConstantPropertyLiquidSodium, R=1e6)
    annotation (Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=180,
        origin={-30,26})));
equation
  ...
  connect(boundary9.ports[1:3], STHX.port_b_tube) annotation (Line(points={{98,-4.66667},
          {24,-4.66667},{24,-20}}, color={0,127,255}));
  connect(boundary7.ports[1], volume.port_b[1])
    annotation (Line(points={{100,-100},{92,-100}}, color={0,127,255}));
  connect(resistances1.port_b, STHX.port_a_tube) annotation (Line(points={{47,-100},
          {24,-100},{24,-40}}, color={0,127,255}));
  connect(volume.port_a[1:3], resistances1.port_a) annotation (Line(points={{80,
          -99.3333},{71,-99.3333},{71,-100},{61,-100}}, color={0,127,255}));
  ...
  annotation (
    ...
    Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-180,-180},{200,
            140}})),
