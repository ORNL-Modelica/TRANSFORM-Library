...
model SFR
  ...
  Fluid.BoundaryConditions.MassFlowSource_T boundary4(         redeclare
    ...
    annotation (Placement(transformation(extent={{-86,-74},{-106,-54}})));
  ...
  Fluid.BoundaryConditions.MassFlowSource_T boundary1(         redeclare
      package Medium = Medium,
    nPorts=1,
    T=data.T_start_hot,
    m_flow=data.m_flow_PHTS)
  ...
  HeatExchangers.GenericDistributed_HX STHX(
    ...
    T_b_start_tube=data.T_IHX_outletIHTS) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={120,6})));
...
equation
  ...
  connect(boundary4.ports[1], shield.port_a) annotation (Line(points={{-106,-64},
          {-64,-64},{-64,-50},{-70,-50},{-70,-10}}, color={0,127,255}));
  connect(boundary5.ports[1], reflector.port_a) annotation (Line(points={{-48,
          -74},{-44,-74},{-44,-10},{-40,-10}}, color={0,127,255}));
  connect(shield.port_b, upperPlenum.port_a[1]) annotation (Line(points={{-70,
          10},{-70,10},{-70,44},{-9.25,44}}, color={0,127,255}));
  connect(reflector.port_b, upperPlenum.port_a[2]) annotation (Line(points={{
          -40,10},{-40,44},{-9.75,44}}, color={0,127,255}));
  ...
  connect(innerCore.port_b, innerCore_out.port_a)
    annotation (Line(points={{18,4},{18,12}}, color={0,127,255}));
  connect(innerCore_out.port_b, upperPlenum.port_a[4])
    annotation (Line(points={{18,32},{18,44},{-10.75,44}}, color={0,127,255}));
  connect(boundary2.ports[1], innerCore_in.port_a)
    annotation (Line(points={{42,-72},{18,-72},{18,-44}}, color={0,127,255}));
  connect(innerCore_in.port_b, innerCore.port_a)
    annotation (Line(points={{18,-24},{18,-16}}, color={0,127,255}));
  connect(upperPlenum.port_b[1], expansionTank.port_a)
    annotation (Line(points={{-10,56},{-10,68},{0,68}}, color={0,127,255}));
  connect(expansionTank.port_b, boundary6.ports[1])
    annotation (Line(points={{12,68},{32,68},{32,72}}, color={0,127,255}));
  connect(boundary1.ports[1], STHX.port_a_shell) annotation (Line(points={{92,
          56},{115.4,56},{115.4,16}}, color={0,127,255}));
