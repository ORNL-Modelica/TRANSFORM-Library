...
replaceable package Medium =
 ...
 replaceable package Medium_Ambient =
  ...
  Fluid.Pipes.GenericPipe_withWall pipe(
    ...
    p_a_start=250000,
    counterCurrent=true)               annotation (Placement(transformation(
        extent={{10,10},{-10,-10}},
        rotation=90,
        origin={50,-10})));
  ...
  Fluid.Pipes.GenericPipe_withWall pipe1(
    ...
    p_a_start=350000,
    T_a1_start=data.T_outlet_DRACSsec,
    T_a2_start=data.T_outlet_DRACSsec) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={-100,-10})));

  HeatAndMassTransfer.BoundaryConditions.Heat.HeatFlow boundary1[2](Q_flow=750e3
        /2) annotation (Placement(transformation(extent={{-136,-20},{-116,0}})));
  ...
  Modelica.Fluid.Sources.MassFlowSource_T blower(
    ...
    use_m_flow_in=true,
    T=298.15,
    nPorts=1) annotation (Placement(transformation(extent={{114,-62},{94,-42}})));
  Fluid.BoundaryConditions.Boundary_pT atmosphere(
    p=100000,
    T=293.15,
    redeclare package Medium = Medium_Ambient,
    nPorts=1) annotation (Placement(transformation(extent={{110,18},{90,38}})));
  Fluid.Pipes.GenericPipe                      pipe2(
    ...
    redeclare model Geometry =
        ...
        rotation=90,
        origin={80,-10})));
  Fluid.Sensors.Temperature temperature(redeclare package Medium = Medium)
  ...
  Controls.LimPID PID(
    ...
    yMin=0,
    yb=2,
    k=-10) annotation (Placement(transformation(extent={{150,-54},{130,-34}})));
  Modelica.Blocks.Sources.Constant const(k=data.T_inlet_DRACSsec)
    annotation (Placement(transformation(extent={{168,-20},{148,0}})));
  Fluid.Sensors.MassFlowRate massFlowRate(redeclare package Medium =
        Medium_Ambient, precision=3) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={76,-40})));
equation
  ...
  connect(temperature.port, pump.port_a) annotation (Line(points={{60,-80},{50,-80},
          {50,-60},{-20,-60}}, color={0,127,255}));
  connect(temperature1.port, pipe_fromDRACS.port_b)
    annotation (Line(points={{60,40},{-50,40}}, color={0,127,255}));
  connect(temperature2.port_a, pipe2.port_b) annotation (Line(points={{90,4},{
          86,4},{86,0},{80,0}},color={0,127,255}));
  connect(temperature2.port_b, atmosphere.ports[1]) annotation (Line(points={{110,
          4},{110,16},{84,16},{84,28},{90,28}}, color={0,127,255}));
  connect(temperature3.port_b, pipe2.port_a) annotation (Line(points={{112,-26},
          {112,-20},{80,-20}}, color={0,127,255}));
  connect(blower.m_flow_in, PID.y)
    annotation (Line(points={{114,-44},{129,-44}}, color={0,0,127}));
  connect(blower.ports[1], massFlowRate.port_a) annotation (Line(points={{94,-52},
          {62,-52},{62,-40},{66,-40}}, color={0,127,255}));
  connect(massFlowRate.port_b, temperature3.port_a) annotation (Line(points={{86,
          -40},{90,-40},{90,-26},{92,-26}}, color={0,127,255}));
  connect(temperature.T, PID.u_m)
    annotation (Line(points={{71,-70},{140,-70},{140,-56}}, color={0,0,127}));
  connect(const.y, PID.u_s) annotation (Line(points={{147,-10},{140,-10},{140,-30},
          {160,-30},{160,-44},{152,-44}}, color={0,0,127}));
  connect(pipe.heatPorts, pipe2.heatPorts)
    annotation (Line(points={{55,-10},{75,-10}}, color={191,0,0}));
