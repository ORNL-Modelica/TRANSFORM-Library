...
model IRIS
  ...
  Fluid.Volumes.MixingVolume          ChargingMixerVolume(
    ...
    redeclare model Geometry =
        Fluid.ClosureRelations.Geometry.Models.LumpedVolume.GenericVolume (V=
            1.5))                        annotation (Placement(transformation(
        extent={{-6,-6},{6,6}},
        rotation=90,
        origin={-60,40})));
  ...
  TRANSFORM.Fluid.FittingsAndResistances.SpecifiedResistance ToPressurizer_PressureDrop(R=1, redeclare package Medium =
                       Medium_PHTS) annotation (Placement(transformation(
        extent={{5,-5},{-5,5}},
        rotation=90,
        origin={-28,63})));
  ...
  TRANSFORM.Fluid.FittingsAndResistances.SpecifiedResistance Core_InletPressureDrop(R=7*11000/(8*589),
      redeclare package Medium = Medium_PHTS) annotation (Placement(
        ...
        rotation=90,
        origin={-60,-74.5})));
  TRANSFORM.Fluid.FittingsAndResistances.SpecifiedResistance ToHeader_PressureDrop(R=1/(5*589), redeclare package Medium =
                       Medium_PHTS) annotation (Placement(transformation(
        extent={{5,-5},{-5,5}},
        rotation=180,
        origin={-48,52})));
  ...
  TRANSFORM.Fluid.FittingsAndResistances.SpecifiedResistance ToHeader_PressureDrop1(
                                                                                   R=1/(5*589), redeclare package Medium =
                       Medium_PHTS) annotation (Placement(transformation(
        extent={{5,-5},{-5,5}},
        rotation=180,
        origin={-14,52})));
...
equation
  ...
  connect(Steam_InletPipe.port_a,port_a)
    annotation (Line(points={{80,-40},{100,-40}}, color={0,127,255}));
  connect(sensorBus.PHS.p_pressurizer, p_pressurizer.y) annotation (Line(
  ...
  connect(actuatorBus.PHS.speedPump, pump.inputSignal) annotation (Line(
  ...
  connect(UpperRiser.port_b, ChargingMixerVolume.port_a[1])
    annotation (Line(points={{-60,26},{-60,36.4}}, color={0,127,255}));
  connect(DownComer.port_b, LowerPlenum.port_a[1]) annotation (Line(points={{0,
          -76},{0,-76},{0,-84},{-26.4,-84}}, color={0,127,255}));
  ...
  connect(T_Core_Inlet.port, Core_InletPressureDrop.port_b) annotation (Line(
        points={{-72,-68},{-60,-68},{-60,-70.65}}, color={0,127,255}));
  connect(vaporHeaterSource.y, vaporHeater.Q_flow_ext) annotation (Line(points=
          {{-50.6,83},{-47.3,83},{-47.3,83},{-44.2,83}}, color={0,0,127}));
  connect(actuatorBus.PHS.Q_flow_liquidHeater, liquidHeater.Q_flow_ext)
    annotation (Line(
      ...
      pattern=LinePattern.Dash,
      thickness=0.5));
  connect(sensorBus.PHS.Q_total, Q_total.y) annotation (Line(
      points={{-29.9,100.1},{-29.9,100.1},{-60,100.1},{-60,124},{-63.4,124}},
      color={239,82,82},
      pattern=LinePattern.Dash,
      thickness=0.5));
  connect(actuatorBus.PHS.reactivity_ControlRod, CR_reactivity.u) annotation (
      Line(
      points={{30.1,100.1},{30.1,100.1},{-2,100.1},{-2,102},{-55.2,102},{-55.2,134}},
      color={111,216,99},
      pattern=LinePattern.Dash,
      thickness=0.5));
  annotation (
