...
model GenericModule
  ...
  TRANSFORM.Fluid.FittingsAndResistances.SpecifiedResistance tee_inlet(
      ...
      Placement(transformation(
        ...
        rotation=0,
        origin={-30,40})));
  TRANSFORM.Fluid.Machines.Pump_SimpleMassFlow pump_SimpleMassFlow1(redeclare package
              Medium = Medium, m_flow_nominal=data.m_flow_steam,
    use_input=true)
  ...
  TRANSFORM.Fluid.Sensors.RelativeTemperature relativeTemperature(redeclare package
              Medium = Medium_PHTS, refPort_a=false) annotation (Placement(
        transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={-66,-42})));
