...
if energyDynamics == Modelica.Fluid.Types.Dynamics.SteadyState then
    ...
    for i in 2:nR - 1 loop
      ...
      0 =-0.5*(lambda[i, j] + lambda[i - 1, j])*2*pi*(rs[i] - 0.5*dr[i - 1])
          ...
          *(lambda[i, j] + lambda[i + 1, j])*2*pi*(rs[i] + 0.5*dr[i])*0.5*
          (dz[j] + dz[j - 1])*(Ts[i, j] - Ts[i + 1, j])/dr[i] + q_ppp[i,
          j]*Vs[i, j];
    end for;
  end for;
  /* Inner Edge */
    ...
    for j in 1:1 loop
      ...
      0 =-0.5*(lambda[i, j] + lambda[i - 1, j])*2*pi*(rs[i] - 0.5*dr[i - 1])
          *0.5*dz[j]*(Ts[i, j] - Ts[i - 1, j])/dr[i - 1] - 0.5*(lambda[i,
          ...
          j] - Ts[i + 1, j])/dr[i] + heatPorts_bottom[i].Q_flow + q_ppp[i,
          j]*Vs[i, j];
    end for;
  end for;
  /* Top Edge */
    for i in 2:nR - 1 loop
      ...
      0 =-0.5*(lambda[i, j] + lambda[i - 1, j])*2*pi*(rs[i] - 0.5*dr[i - 1])
          *0.5*dz[j - 1]*(Ts[i, j] - Ts[i - 1, j])/dr[i - 1] - 0.5*(
          lambda[i, j] + lambda[i + 1, j])*2*pi*(rs[i] + 0.5*dr[i])*0.5*
          dz[j - 1]*(Ts[i, j] - Ts[i + 1, j])/dr[i] + heatPorts_top[i].Q_flow
           + q_ppp[i, j]*Vs[i, j];
    end for;
  end for;
  /* Bottom Inner Corner */
    ...
    for j in 1:1 loop
      ...
      0 =-0.5*(lambda[i, j] + lambda[i - 1, j])*2*pi*(rs[i] - 0.5*dr[i - 1])
          *0.5*dz[j]*(Ts[i, j] - Ts[i - 1, j])/dr[i - 1] +
          heatPorts_outer[j].Q_flow + heatPorts_bottom[i].Q_flow + q_ppp[
          i, j]*Vs[i, j];
    end for;
  end for;
  /* Top Outer Corner */
    for i in nR:nR loop
      ...
      0 =-0.5*(lambda[i, j] + lambda[i - 1, j])*2*pi*(rs[i] - 0.5*dr[i - 1])
          *0.5*dz[j - 1]*(Ts[i, j] - Ts[i - 1, j])/dr[i - 1] +
          heatPorts_outer[j].Q_flow + heatPorts_top[i].Q_flow + q_ppp[i,
          j]*Vs[i, j];
    end for;
  end for;
else
  ...
  for i in 1:1 loop
      ...
      A_inner[j] =2*pi*rs[i]*0.5*(dz[j] + dz[j - 1]);
        ...
        d[i, j]*cp[i, j]*Vs[i, j]*der(Ts[i, j]) = heatPorts_inner[j].Q_flow
          ...
          *0.5*(dz[j] + dz[j - 1])*(Ts[i, j] - Ts[i + 1, j])/dr[i] +
          q_ppp[i, j]*Vs[i, j];
    end for;
  end for;
  /* Outer Edge */
    ...
    for i in 2:nR - 1 loop
      ...
      A_top[i] =pi*(rs[i]*(dr[i] + dr[i - 1]) + 0.25*(dr[i]*dr[i] - dr[i -
        ...
        d[i, j]*cp[i, j]*Vs[i, j]*der(Ts[i, j]) = -0.5*(lambda[i, j] +
          ...
          Ts[i + 1, j])/dr[i] + heatPorts_top[i].Q_flow + q_ppp[i, j]*Vs[
          i, j];
    end for;
  end for;
  /* Bottom Inner Corner */
  ...
  for i in 1:1 loop
      ...
      heatPorts_top[i].T =Ts[i, j];
        d[i, j]*cp[i, j]*Vs[i, j]*der(Ts[i, j]) = heatPorts_inner[j].Q_flow
           - 0.5*(lambda[i, j] + lambda[i + 1, j])*2*pi*(rs[i] + 0.5*dr[i])
          *0.5*dz[j - 1]*(Ts[i, j] - Ts[i + 1, j])/dr[i] + heatPorts_top[
