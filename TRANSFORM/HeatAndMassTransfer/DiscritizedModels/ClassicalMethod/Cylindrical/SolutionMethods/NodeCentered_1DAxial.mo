...
if energyDynamics == Modelica.Fluid.Types.Dynamics.SteadyState then
  ...
  for i in 1:1 loop
      ...
      0 =heatPorts_inner[j].Q_flow - 0.5*(lambda[i, j] + lambda[i, j - 1])
          *pi*(rs[i]*dr[i] + 0.25*dr[i]*dr[i])*(Ts[i, j] - Ts[i, j - 1])/
          dz[j - 1] - 0.5*(lambda[i, j] + lambda[i, j + 1])*pi*(rs[i]*dr[
          i] + 0.25*dr[i]*dr[i])*(Ts[i, j] - Ts[i, j + 1])/dz[j] + q_ppp[
          i, j]*Vs[i, j];
    end for;
  end for;
    ...
    for j in 1:1 loop
      ...
      0 =+heatPorts_bottom[i].Q_flow - 0.5*(lambda[i, j] + lambda[i, j + 1])
          *pi*(rs[i]*(dr[i] + dr[i - 1]) + 0.25*(dr[i]*dr[i] - dr[i - 1]*
          dr[i - 1]))*(Ts[i, j] - Ts[i, j + 1])/dz[j] + q_ppp[i, j]*Vs[i,
          j];
    end for;
  end for;
  ...
  for i in 1:1 loop
      ...
      A_inner[j] =2*pi*rs[i]*0.5*(dz[j] + dz[j - 1]);
        ...
        d[i, j]*cp[i, j]*Vs[i, j]*der(Ts[i, j]) = heatPorts_inner[j].Q_flow
          ...
          dr[i]*dr[i])*(Ts[i, j] - Ts[i, j - 1])/dz[j - 1] - 0.5*(lambda[
          i, j] + lambda[i, j + 1])*pi*(rs[i]*dr[i] + 0.25*dr[i]*dr[i])*(
  ...
  for i in 1:1 loop
      ...
      heatPorts_top[i].T =Ts[i, j];
        d[i, j]*cp[i, j]*Vs[i, j]*der(Ts[i, j]) = heatPorts_inner[j].Q_flow
           - 0.5*(lambda[i, j] + lambda[i, j - 1])*pi*(rs[i]*dr[i] + 0.25*
          dr[i]*dr[i])*(Ts[i, j] - Ts[i, j - 1])/dz[j - 1] +
          heatPorts_top[i].Q_flow + q_ppp[i, j]*Vs[i, j];
    end for;
  end for;
