...
if energyDynamics == Modelica.Fluid.Types.Dynamics.SteadyState then
    ...
    for i in 2:nR - 1 loop
      for j in 2:nZ - 1 loop
        Vs[i, j] = pi*(rs[i]*(dr[i] + dr[i - 1]) + 0.25*(dr[i]*dr[i] - dr[
          i - 1]*dr[i - 1]))*dz[j];
      0 =-0.5*(lambda[i, j] + lambda[i, j - 1])*pi*(rs[i]*(dr[i] + dr[i -
  ...
  for i in 1:1 loop
      ...
      0 =heatPorts_inner[j].Q_flow - 0.5*(lambda[i, j] + lambda[i, j - 1])
          *pi*(rs[i]*dr[i] + 0.25*dr[i]*dr[i])*(Ts[i, j] - Ts[i, j - 1])/(
          0.5*(dz[j] + dz[j - 1])) - 0.5*(lambda[i, j] + lambda[i, j + 1])
          *pi*(rs[i]*dr[i] + 0.25*dr[i]*dr[i])*(Ts[i, j] - Ts[i, j + 1])/(
  ...
  /* Outer Edge */
    for i in nR:nR loop
      ...
      0 =+heatPorts_outer[j].Q_flow - 0.5*(lambda[i, j] + lambda[i, j - 1])
          *pi*(rs[i]*dr[i - 1] - 0.25*dr[i - 1]*dr[i - 1])*(Ts[i, j] - Ts[
          i, j - 1])/(0.5*(dz[j] + dz[j - 1])) - 0.5*(lambda[i, j] +
          lambda[i, j + 1])*pi*(rs[i]*dr[i - 1] - 0.25*dr[i - 1]*dr[i - 1])
          *(Ts[i, j] - Ts[i, j + 1])/(0.5*(dz[j] + dz[j + 1])) + q_ppp[i,
          j]*Vs[i, j];
    end for;
  end for;
  /* Bottom Edge */
    ...
    for j in 1:1 loop
      ...
      0 =+heatPorts_bottom[i].Q_flow - 0.5*(lambda[i, j] + lambda[i, j + 1])
  ...
  for i in 1:1 loop
    for j in 1:1 loop
      ...
      0 =heatPorts_inner[j].Q_flow + heatPorts_bottom[i].Q_flow - 0.5*(
          lambda[i, j] + lambda[i, j + 1])*pi*(rs[i]*dr[i] + 0.25*dr[i]*
          dr[i])*(Ts[i, j] - Ts[i, j + 1])/(0.5*(dz[j] + dz[j + 1])) +
          q_ppp[i, j]*Vs[i, j];
    end for;
  end for;
  /* Top Inner Corner */
  for i in 1:1 loop
      ...
      0 =heatPorts_inner[j].Q_flow - 0.5*(lambda[i, j] + lambda[i, j - 1])
          *pi*(rs[i]*dr[i] + 0.25*dr[i]*dr[i])*(Ts[i, j] - Ts[i, j - 1])/(
          0.5*(dz[j] + dz[j - 1])) + heatPorts_top[i].Q_flow + q_ppp[i, j]
          *Vs[i, j];
    end for;
  end for;
  /* Bottom Outer Corner */
    ...
    for j in 1:1 loop
      ...
      A_bottom[i] =pi*(rs[i]*dr[i - 1] - 0.25*dr[i - 1]*dr[i - 1]);
        ...
        Ts[i, j] = heatPorts_outer[j].T;
      heatPorts_bottom[i].T =Ts[i, j];
      0 =+heatPorts_outer[j].Q_flow + heatPorts_bottom[i].Q_flow - 0.5*(
    ...
    for i in 2:nR - 1 loop
      for j in 2:nZ - 1 loop
        ...
        d[i, j]*cp[i, j]*Vs[i, j]*der(Ts[i, j]) = -0.5*(lambda[i, j] +
          lambda[i, j - 1])*pi*(rs[i]*(dr[i] + dr[i - 1]) + 0.25*(dr[i]*
          dr[i] - dr[i - 1]*dr[i - 1]))*(Ts[i, j] - Ts[i, j - 1])/(0.5*(
          dz[j] + dz[j - 1])) - 0.5*(lambda[i, j] + lambda[i, j + 1])*pi*(
          rs[i]*(dr[i] + dr[i - 1]) + 0.25*(dr[i]*dr[i] - dr[i - 1]*dr[i -
          1]))*(Ts[i, j] - Ts[i, j + 1])/(0.5*(dz[j] + dz[j + 1])) +
          q_ppp[i, j]*Vs[i, j];
    end for;
  end for;
  /* Inner Edge */
  ...
  /* Outer Edge */
    for i in nR:nR loop
      ...
      A_outer[j] =2*pi*rs[i]*dz[j];
        Vs[i, j] = pi*(rs[i]*dr[i - 1] - 0.25*dr[i - 1]*dr[i - 1])*dz[j];
        Ts[i, j] = heatPorts_outer[j].T;
        d[i, j]*cp[i, j]*Vs[i, j]*der(Ts[i, j]) = +heatPorts_outer[j].Q_flow
    ...
    for j in 1:1 loop
      A_bottom[i] =pi*(rs[i]*(dr[i] + dr[i - 1]) + 0.25*(dr[i]*dr[i] - dr[
        ...
        Vs[i, j] = pi*(rs[i]*(dr[i] + dr[i - 1]) + 0.25*(dr[i]*dr[i] - dr[
          i - 1]*dr[i - 1]))*dz[j];
        Ts[i, j] = heatPorts_bottom[i].T;
        d[i, j]*cp[i, j]*Vs[i, j]*der(Ts[i, j]) = +heatPorts_bottom[i].Q_flow
           - 0.5*(lambda[i, j] + lambda[i, j + 1])*pi*(rs[i]*(dr[i] + dr[
          i - 1]) + 0.25*(dr[i]*dr[i] - dr[i - 1]*dr[i - 1]))*(Ts[i, j] -
    ...
    for i in 2:nR - 1 loop
      ...
      A_top[i] =pi*(rs[i]*(dr[i] + dr[i - 1]) + 0.25*(dr[i]*dr[i] - dr[i -
        ...
        d[i, j]*cp[i, j]*Vs[i, j]*der(Ts[i, j]) = -0.5*(lambda[i, j] +
          lambda[i, j - 1])*pi*(rs[i]*(dr[i] + dr[i - 1]) + 0.25*(dr[i]*
          dr[i] - dr[i - 1]*dr[i - 1]))*(Ts[i, j] - Ts[i, j - 1])/(0.5*(
          dz[j] + dz[j - 1])) + heatPorts_top[i].Q_flow + q_ppp[i, j]*Vs[
          i, j];
    end for;
  end for;
  /* Bottom Inner Corner */
    ...
    for j in 1:1 loop
      ...
      heatPorts_bottom[i].T =Ts[i, j];
        d[i, j]*cp[i, j]*Vs[i, j]*der(Ts[i, j]) = heatPorts_inner[j].Q_flow
          ...
          1])*pi*(rs[i]*dr[i] + 0.25*dr[i]*dr[i])*(Ts[i, j] - Ts[i, j + 1])
          /(0.5*(dz[j] + dz[j + 1])) + q_ppp[i, j]*Vs[i, j];
    end for;
  end for;
  /* Top Inner Corner */
