...
model Winding_123D_variableArea
  import TRANSFORM;
  ...
  Modelica.Blocks.Sources.IntegerConstant nNodes_3(k=15)
  ...
  Modelica.Blocks.Sources.Sine r_outer(
    ...
    amplitude=0.005,
    startTime=10)
    annotation (Placement(transformation(extent={{-100,52},{-90,62}})));
  Modelica.Blocks.Sources.Sine length_z(
    offset=0.03,
    freqHz=1/50,
    amplitude=0.01,
    startTime=20)
    annotation (Placement(transformation(extent={{-100,34},{-90,44}})));
...
equation
  ...
  connect(bound_inner.port, winding.port_a2)
    annotation (Line(points={{0,-28},{0,-10}},         color={191,0,0}));
  annotation (Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-100,
            -100},{100,100}})),
    experiment(StopTime=100, __Dymola_NumberOfIntervals=100));
end Winding_123D_variableArea;
