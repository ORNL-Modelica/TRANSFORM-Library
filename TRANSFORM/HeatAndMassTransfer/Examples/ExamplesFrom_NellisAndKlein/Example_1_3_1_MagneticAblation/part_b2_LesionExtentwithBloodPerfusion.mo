...
model part_b2_LesionExtentwithBloodPerfusion
  "Part b) Locate the location where the tissue first reaches the lethal temperature with blood perfusion see 1.8-2 pg 161-165"
  import TRANSFORM;
  ...
  DiscritizedModels.Conduction_1D tissue(
    ...
    redeclare model InternalHeatModel =
        DiscritizedModels.BaseClasses.Dimensions_1.GenericHeatGeneration (
          Q_gens={-beta.y*tissue.geometry.Vs[i]*(tissue.materials[i].T -
            Tissue_infinite.T) for i in 1:nNodes_1_t.k}))
  ...
  Modelica.Blocks.Sources.Constant beta(each k=20000)
...
equation
  ...
  connect(tissue.port_b1, Tissue_infinite.port)
    annotation (Line(points={{30,0},{74,0}},        color={191,0,0}));
...
end part_b2_LesionExtentwithBloodPerfusion;
