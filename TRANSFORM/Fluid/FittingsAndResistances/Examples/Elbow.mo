...
model Elbow
  ...
  TRANSFORM.Fluid.FittingsAndResistances.Elbow from_dp(
  ...
  TRANSFORM.Fluid.FittingsAndResistances.Elbow from_mflow(
    redeclare package Medium = Modelica.Media.Air.DryAirNasa,
    dimension=0.05,
    radius=radius.y)
    annotation (Placement(transformation(extent={{-10,-50},{10,-30}})));
  Modelica.Blocks.Sources.Ramp radius(
    offset=0.025,
    startTime=0,
    height=0.1,
    duration=0.5) "Bend radius -> relative radius rc/D ramps from 0.5 to 2.5"
...
<p>Unit test for <a href=\"modelica://TRANSFORM.Fluid.FittingsAndResistances.Elbow\">Elbow</a>. The bend radius is
ramped so the relative bend radius <code>rc/D</code> sweeps from 0.5 to 2.5 during the run. Two parallel circuits are
exercised: <code>from_dp</code> is pressure-driven and <code>from_mflow</code> is driven by a sinusoidal (sign-changing)
mass flow rate, verifying both the loss magnitude and correct flow-reversal behaviour.</p>
</html>"));
end Elbow;
