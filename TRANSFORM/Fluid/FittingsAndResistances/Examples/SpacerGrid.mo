...
model SpacerGrid
  ...
  TRANSFORM.Fluid.FittingsAndResistances.SpacerGrid from_dp(
    redeclare package Medium = Modelica.Media.Air.DryAirNasa,
    epsilon=0.3,
    crossArea=1e-3,
    dimension=0.012) "30% blockage spacer grid"
  ...
  Modelica.Blocks.Sources.Sine input_mflow(
    f=1,
    offset=0.02,
    amplitude=0.01) annotation (Placement(transformation(extent={{-100,-50},{-80,
            -30}})));
  inner Modelica.Fluid.System system(p_ambient(displayUnit="Pa") = 100000,
      m_flow_small=0.001)
    annotation (Placement(transformation(extent={{80,-100},{100,-80}})));
  TRANSFORM.Fluid.FittingsAndResistances.SpacerGrid from_mflow(
    redeclare package Medium = Modelica.Media.Air.DryAirNasa,
    epsilon=0.3,
    crossArea=1e-3,
    dimension=0.012)
...
<p>Unit test for <a href=\"modelica://TRANSFORM.Fluid.FittingsAndResistances.SpacerGrid\">SpacerGrid</a> (Rehme, 30%
blockage). <code>from_dp</code> is pressure-driven; <code>from_mflow</code> uses an offset sinusoidal flow so the
Reynolds-number dependence of the drag coefficient is exercised.</p>
</html>"));
end SpacerGrid;
