...
model Nus_McCartyWolf_Simple
  "McCarthy-Wolf | Simple | A*Re^B*Pr^C*(Tw/Tb)^D"
  ...
  input Real Cs0[nHT,nSurfaces]=fill(
      ...
      nHT,
      nSurfaces) "if non-uniform then set" annotation (Dialog(group="Inputs"));
  input Real D0=-0.55 "Exponent to Temperature ratio"
    annotation (Dialog(group="Inputs"));
...
equation
  for i in 1:nHT loop
    for j in 1:nSurfaces loop
      Nus[i, j] = As0[i, j]*Res[i]^Bs0[i, j]*Prs[i]^Cs0[i, j]*(Ts_wall[i, j]/T_bulk[i, j])^Ds0[i, j];
    end for;
  end for;
