...
partial model PartialHeatTransfer_setQ_flows
  ...
  parameter Boolean use_RadHT = false "=true to turn on radiative heat transfer" annotation(Evaluate=true);
  input SI.Emissivity epsilon = 1 "Emissivity" annotation(Dialog(group="Inputs",enable=use_RadHT));
  input SI.Emissivity epsilons[nHT,nSurfaces] = fill(epsilon,nHT,nSurfaces) "if non-uniform then set" annotation(Dialog(group="Inputs",enable=use_RadHT));

  //SI.ThermalResistance R[nHT,nSurfaces] "Radiative heat resistance";
  SI.HeatFlowRate Q_flows_radHT[nHT,nSurfaces] "Radiation heat transfer flow rate";
...
equation
  ...
  for i in 1:nHT loop
    for j in 1:nSurfaces loop
      Q_flows_radHT[i,j]= (Ts_wall[i, j] - Ts_fluid[i])*(surfaceAreas[i, j]*sigma*epsilons[i, j]*(Ts_wall[i, j]^2 - Ts_fluid[i]^2)*(Ts_wall[i, j] - Ts_fluid[i]));
    end for;
  end for;
  else
