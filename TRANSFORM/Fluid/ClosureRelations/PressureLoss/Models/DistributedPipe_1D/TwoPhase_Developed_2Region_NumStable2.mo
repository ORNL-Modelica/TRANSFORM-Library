...
model TwoPhase_Developed_2Region_NumStable2
  ...
  SI.DynamicViscosity mus_2phase[nFM+1];
  SI.DynamicViscosity mus_mod[nFM+1];
protected
  TRANSFORM.Fluid.ClosureRelations.PressureLoss.Functions.TubesAndConduits.SinglePhase.LaminarTurbulent_MSLDetailed.dp_IN_var
    IN_var_nominal(
    rho_a=d_nominal,
...
equation
  ...
  for i in 1:nFM+1 loop
    mus_2phase[i] = mediaProps[i].mu_lsat*(1 - mediaProps[i].alphaV) .* (1 + 2.5
      *mediaProps[i].alphaV) + mediaProps[i].alphaV*mediaProps[i].mu_vsat;
    mus_mod[i] = TRANSFORM.Math.spliceTanh(
      TRANSFORM.Math.spliceTanh(
        mediaProps[i].mu,
        mus_2phase[i],
        mediaProps[i].alphaV - 1.0,
        0.001),
      mediaProps[i].mu,
      mediaProps[i].alphaV,
      0.0001);
...
end TwoPhase_Developed_2Region_NumStable2;
