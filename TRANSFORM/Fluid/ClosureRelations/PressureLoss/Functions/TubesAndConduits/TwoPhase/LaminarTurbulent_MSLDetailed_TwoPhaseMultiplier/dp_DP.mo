within TRANSFORM.Fluid.ClosureRelations.PressureLoss.Functions.TubesAndConduits.TwoPhase.LaminarTurbulent_MSLDetailed_TwoPhaseMultiplier;
...
algorithm
  ...
  rho_l     :=if m_flow >= 0 then IN_var.rho_l_a else IN_var.rho_l_b;
  mu_l      :=if m_flow >= 0 then IN_var.mu_l_a else IN_var.mu_l_b;
  rho_v     :=if m_flow >= 0 then IN_var.rho_v_a else IN_var.rho_v_b;
  mu_v      :=if m_flow >= 0 then IN_var.mu_v_a else IN_var.mu_v_b;
  x_abs      :=if m_flow >= 0 then IN_var.x_abs_a else IN_var.x_abs_b;
