...
algorithm
  ...
  Re := lambda2*phi2/64 "Hagen-Poiseuille";
  dRe_ddp := aux1*phi2/64 "Hagen-Poiseuille";

  // Modify Re, if turbulent flow
  if Re > Re1 then
    Re := -2*sqrt(lambda2*phi2)*Modelica.Math.log10(2.51/sqrt(lambda2*phi2) + 0.27*Delta)
      "Colebrook-White";
    aux2 := sqrt(aux1*phi2*abs(dp_fric));
    dRe_ddp := 1/log(10)*(-2*log(2.51/aux2+0.27*Delta)*aux1*phi2/(2*aux2)+2*2.51/(2*abs(dp_fric)*(2.51/aux2+0.27*Delta)));
    if Re < Re2 then
      (Re, dRe_ddp) := interpolateInRegion2_withDerivative(lambda2*phi2, Re1, Re2, Delta, dp_fric);
    end if;
  end if;
