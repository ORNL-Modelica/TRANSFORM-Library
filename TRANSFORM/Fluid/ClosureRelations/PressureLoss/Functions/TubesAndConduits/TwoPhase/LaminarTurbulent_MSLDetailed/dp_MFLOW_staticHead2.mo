...
algorithm
    ...
    if dp>=dp_a then
      ...
      (M_FLOW,dm_flow_ddp_fric_a,phi2) := Internal.m_flow_of_dp_fric2(IN_con, IN_var, dp-dp_grav_a, Re1, Re2, Delta);
    elseif dp<=dp_b then
      // Negative flow outside regularization
      (M_FLOW,dm_flow_ddp_fric_b,phi2) := Internal.m_flow_of_dp_fric2(IN_con, IN_var, dp-dp_grav_b, Re1, Re2, Delta);
    else
      // Regularization parameters
      (m_flow_a, dm_flow_ddp_fric_a,phi2) := Internal.m_flow_of_dp_fric2(IN_con, IN_var, dp_a-dp_grav_a, Re1, Re2, Delta);
