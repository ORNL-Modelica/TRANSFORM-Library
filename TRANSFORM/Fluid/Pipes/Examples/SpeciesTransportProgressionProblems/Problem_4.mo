...
model Problem_4
  ...
  parameter SI.Length length_loop=0.300;
  ...
  parameter Real beta_i[nC] = fill(1,nC);//{0.0006,0.00364,0.00349,0.00628,0.00179,0.00070};
  ...
  final parameter SIadd.ExtraProperty Cs_start[nV,nC]={{C_i_start[i, j]/
      ...
      :nV};
  final parameter SI.Length x[nV]=linspace(0,length,nV);//pipe.summary.xpos;
  final parameter SI.Length x_loop[nV]=linspace(0,0.3,
                                                     nV);
  ...
  SIadd.ExtraPropertyFlowRate[nV,nC] mC_gens={{-lambda_i[j]*pipe.mCs[i, j]*pipe.nParallel + beta_i[j]*phi_0*sin(Modelica.Constants.pi*pipe.summary.xpos_norm[j])
      for j in 1:nC} for i in 1:nV};
  SIadd.ExtraPropertyFlowRate[nV,nC] mC_gens_loop={{-lambda_i[j]*pipe_loop.mCs[i, j]*pipe_loop.nParallel
      for j in 1:nC} for i in 1:nV};
        Real cool = v*time;
...
equation
  ...
  // Analytical Solution
  for j in 1:nV loop
    if length+x_loop[j] < v*time then
      for i in 1:nC loop
    ...
    if x[j] < v*time then
      for i in 1:nC loop
         C_i_analytical[j,i]=
          -((-exp((x[j]*lambda_i[i])/v)*Modelica.Constants.pi*v*beta_i[i]*phi_0*cos(Modelica.Constants.pi*(time*v - x[j]))
           + exp((time + x[j]/v)*lambda_i[i])*Modelica.Constants.pi*v*beta_i[i]*phi_0*cos(Modelica.Constants.pi*x[j])
           - exp(time*lambda_i[i])*C_i_loop_analytical[end,i]*Modelica.Constants.pi^2*v^2
           - exp(time*lambda_i[i])*Modelica.Constants.pi*v*beta_i[i]*phi_0
           - exp(time*lambda_i[i])*C_i_loop_analytical[end,i]*lambda_i[i]^2
           + exp((x[j]*lambda_i[i])/v)*Modelica.Constants.pi*v*beta_i[i]*phi_0*cos(Modelica.Constants.pi*(time*v - x[j]))
           - exp((x[j]*lambda_i[i])/v)*beta_i[i]*phi_0*lambda_i[i]*sin(Modelica.Constants.pi*(time*v - x[j]))
           + exp((x[j]*lambda_i[i])/v)*beta_i[i]*phi_0*lambda_i[i]*sin(Modelica.Constants.pi*(time*v - x[j]))
           - exp((time + x[j]/v)*lambda_i[i])*beta_i[i]*phi_0*lambda_i[i]*sin(Modelica.Constants.pi*x[j]))
           /(exp((time + x[j]/v)*lambda_i[i])*(Modelica.Constants.pi^2*v^2 + lambda_i[i]^2)));
      end for;
    else
      for i in 1:nC loop
        C_i_analytical[j,i]=
  ...
  connect(boundary.ports[1], pipe.port_a)
    annotation (Line(points={{-48,0},{-30,0}}, color={0,127,255}));
  ...
  connect(sensor_C.C, boundary.C_in) annotation (Line(points={{50,-3.6},{50,-20},{-80,-20},{-80,-8},{-68,-8}}, color={0,0,127}));
  annotation (
