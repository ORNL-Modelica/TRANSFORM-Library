...
model Problem_6 "Core & Loop | Decay | Advection | Periodic | Generation"
  extends PartialProgressionProblemCoreLoop(v=0.02,mC_gens={{-lambda_i[j]*pipe.mCs[i, j]*pipe.nParallel + 0.01*sin(
...
//
//   SIadd.ExtraPropertyFlowRate[nV,nC] mC_gens={{-lambda_i[j]*pipe.mCs[i, j]*pipe.nParallel + 0.01*sin(
//       Modelica.Constants.pi*pipe.summary.xpos_norm[j]) for j in 1:nC} for i in 1:nV};
//   SIadd.ExtraPropertyFlowRate[nV,nC] mC_gens_pipe={{-lambda_i[j]*pipe_loop.mCs[i, j]*pipe_loop.nParallel for j in 1:nC} for i in 1:nV};
//
...
//   connect(pipe_loop.port_a, pipe.port_b)
