within TRANSFORM.Fluid.Valves.Nozzles.BaseClasses;
partial model PartialNozzle "Base model for nozzles"
  ...
  parameter Modelica.Units.SI.Area Av(fixed=if CvData == Modelica.Fluid.Types.CvTypes.Av
         then true else false, start=m_flow_nominal/(sqrt(rho_nominal*
  ...
  parameter Modelica.Units.SI.Pressure dp_small=dp_nominal/m_flow_nominal*
      m_flow_small "Regularisation of zero flow"
