...
algorithm
  ...
  aux3 :=IN_con.d_o/(mu*A_E)*(IN_con.S/IN_con.S_E);
  ...
  m_flow := mu*A_E/IN_con.d_o*(IN_con.S_E/IN_con.S)*(if dp_fric >= 0 then Re else -Re);

  // Determine derivative of mass flow rate with dp_fric
  dm_flow_ddp_fric := mu*A_E/IN_con.d_o*(IN_con.S_E/IN_con.S)*dRe_ddp;
