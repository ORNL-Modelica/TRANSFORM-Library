...
model DetailedPipeFlow
  ...
  extends TRANSFORM.Fluid.Pipes_Obsolete.ClosureModels.PressureLoss.Models.PartialGenericPipeFlow;
    ...
    if from_dp and not dp_is_zero then
      m_flows = homotopy(actual=
        TRANSFORM.Fluid.Pipes_Obsolete.ClosureModels.PressureLoss.Models.Functions.Pipes.Detailed.dp_MFLOW(
              ...
              dp_small/(nFM))*nParallel, simplified=m_flow_nominal/
        dp_nominal*(dps_fg - g*dheights*rho_nominal));
    else
      dps_fg = homotopy(actual=
        TRANSFORM.Fluid.Pipes_Obsolete.ClosureModels.PressureLoss.Models.Functions.Pipes.Detailed.dp_DP(
              IN_con,
              IN_var,
              m_flows/nParallel,
              m_flow_small/nParallel) + {g*dheights[i]*rhos_act[i] for i in
            1:nFM}, simplified=dp_nominal/m_flow_nominal*m_flows + g*
    ...
    if from_dp and not dp_is_zero then
      m_flows = homotopy(actual=
        TRANSFORM.Fluid.Pipes_Obsolete.ClosureModels.PressureLoss.Models.Functions.Pipes.Detailed.dp_MFLOW_staticHead(
              ...
              dps_fg,
              dp_small/(nFM),
              g*dheights)*nParallel, simplified=m_flow_nominal/dp_nominal
        *(dps_fg - g*dheights*rho_nominal));
    else
      dps_fg = homotopy(actual=
        TRANSFORM.Fluid.Pipes_Obsolete.ClosureModels.PressureLoss.Models.Functions.Pipes.Detailed.dp_DP_staticHead(
              IN_con,
              IN_var,
              m_flows/nParallel,
              m_flow_small/nParallel,
              g*dheights), simplified=dp_nominal/m_flow_nominal*m_flows
